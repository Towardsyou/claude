...
Use the MCP `user-pencil` tools to read and parse .pen files:
...
Always use `flutter create` to generate the project:

```bash
flutter create --platforms=android,ios,macos,web flutter_design_demo
cd flutter_design_demo
rm -rf test/  # Remove default test directory
```
...
4. **Create Project**:
   ...
   cd flutter_design_demo
   rm -rf test/
   mkdir -p lib/components lib/pages
   ```

5. **Set up Demo Shell**:
   - Create `demo_shell.dart` with floating button navigation
